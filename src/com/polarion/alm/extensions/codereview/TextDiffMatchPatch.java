...
public class TextDiffMatchPatch
{
    ...
    protected static class Patch
    {
        ...
        public String toString()
        {
            ...
            return TextDiffMatchPatch.unescapeForEncodeUriCompatability(text.toString());
        }
    }
    ...
    public Object[] patchApply(final List<Patch> patches, final String text)
    {
        ...
        for (Patch aPatch : resultPatches)
        {
            ...
            if (text1.length() > matchMaxBits)
            {
                ...
                if (startLoc != -1)
                {
                    endLoc = matchMain(resultText, text1.substring(text1.length() - matchMaxBits), expectedLoc
                            + text1.length() - matchMaxBits);
            ...
            else
            {
                ...
                } else
                {
                    ...
                    if (text1.length() > matchMaxBits
                            && diffLevenshtein(diffs) / (float) text1.length() > patchDeleteThreshold) {
